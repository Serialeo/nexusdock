...
Bridge 对单个节点串行提交完整快照和连接代次，不同节点使用独立锁。`node.ready` 必须写入成功后，才能将新连接放入可调用目录并执行目录回调，保证首次连接和重连的首条消息都不会被并发调用抢占；确认失败不替换旧连接。网络写入仅占用本节点的锁，同连接重复快照按内容哈希去重。AgentDock 仍在远程开关结果返回前发送最新快照，并在同连接上去重通知；没有取消这项顺序保证。

## 配套升级

本次按 AgentDock `v0.9.4`、NexusDock `v0.9.4` 和共享协议 `v0.10.0` 配套版本统一重部署，不支持混合旧版 AgentDock/Nexus，也不迁移旧开关。热更新管理的最低支持版本为配套的 `v0.9.4`。连接协议版本号本身不能证明节点支持热更新。
